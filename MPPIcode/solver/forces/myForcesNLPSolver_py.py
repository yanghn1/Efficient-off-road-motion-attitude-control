...
a Python wrapper for a fast solver generated by FORCESPRO v5.1.0
   ...
   OUTPUT = myForcesNLPSolver_py.myForcesNLPSolver_solve(PARAMS) solves a multistage problem
   ...
   [OUTPUT, EXITFLAG] = myForcesNLPSolver_py.myForcesNLPSolver_solve(PARAMS) returns additionally
   ...
   [OUTPUT, EXITFLAG, INFO] = myForcesNLPSolver_py.myForcesNLPSolver_solve(PARAMS) returns 
...
#_lib = ctypes.CDLL(os.path.join(os.getcwd(),'myForcesNLPSolver/lib/myForcesNLPSolver.dll')) 
try:
    _lib = ctypes.CDLL(os.path.join(os.path.dirname(os.path.abspath(__file__)),'myForcesNLPSolver/lib/myForcesNLPSolver_withModel.dll'))
    csolver = getattr(_lib,'myForcesNLPSolver_solve')
except:
    _lib = ctypes.CDLL(os.path.join(os.path.dirname(os.path.abspath(__file__)),'myForcesNLPSolver/lib/libmyForcesNLPSolver_withModel.dll'))
    csolver = getattr(_lib,'myForcesNLPSolver_solve')

class myForcesNLPSolver_params_ctypes(ctypes.Structure):
#    @classmethod
#    def from_param(self):
#        return self
    _fields_ = [('lb', ctypes.c_double * 100),
('ub', ctypes.c_double * 100),
('xinit', ctypes.c_double * 3),
('xfinal', ctypes.c_double * 3),
('x0', ctypes.c_double * 253),
('all_parameters', ctypes.c_double * 204),
]

myForcesNLPSolver_params = {'lb' : np.array([]),
'ub' : np.array([]),
'xinit' : np.array([]),
'xfinal' : np.array([]),
'x0' : np.array([]),
...
myForcesNLPSolver_params_types = {'lb' : np.float64,
'ub' : np.float64,
'xinit' : np.float64,
'xfinal' : np.float64,
'x0' : np.float64,
...
class myForcesNLPSolver_outputs_ctypes(ctypes.Structure):
#    @classmethod
#    def from_param(self):
#        return self
    _fields_ = [('MVopt', ctypes.c_double * 100),
('Xopt', ctypes.c_double * 153),
('z', ctypes.c_double * 253),
]

myForcesNLPSolver_outputs = {'MVopt' : np.array([]),
'Xopt' : np.array([]),
'z' : np.array([]),
}


class myForcesNLPSolver_info(ctypes.Structure):
#    @classmethod
#    def from_param(self):
#        return self
...
csolver.argtypes = ( ctypes.POINTER(myForcesNLPSolver_params_ctypes), ctypes.POINTER(myForcesNLPSolver_outputs_ctypes), ctypes.POINTER(myForcesNLPSolver_info), ctypes.POINTER(FILE))
csolver.restype = ctypes.c_int

def myForcesNLPSolver_solve(params_arg):
    '''
a Python wrapper for a fast solver generated by FORCESPRO v5.1.0
 ...
 See also COPYING
    ...
    # convert parameters
    params_py = myForcesNLPSolver_params_ctypes()
    for par in params_arg:
        try:
            ...
            if isinstance(getattr(params_py, par), ctypes.Array):
                params_arg[par] = np.require(params_arg[par], dtype=myForcesNLPSolver_params_types[par], requirements='F')
    ...
    outputs_py = myForcesNLPSolver_outputs_ctypes()
    info_py = myForcesNLPSolver_info()
    if sys.version_info.major == 2:
        ...
        try:
            PyFile_AsFile.restype = ctypes.POINTER(FILE)
            exitflag = _lib.myForcesNLPSolver_solve( ctypes.byref(params_py), ctypes.byref(outputs_py), ctypes.byref(info_py), PyFile_AsFile(fp) , _lib.myForcesNLPSolver_casadi2forces )
        ...
        try:
            if sys.platform.startswith('win'):
                exitflag = _lib.myForcesNLPSolver_solve( ctypes.byref(params_py), ctypes.byref(outputs_py), ctypes.byref(info_py), None , _lib.myForcesNLPSolver_casadi2forces)
            else:
                exitflag = _lib.myForcesNLPSolver_solve( ctypes.byref(params_py), ctypes.byref(outputs_py), ctypes.byref(info_py), fp , _lib.myForcesNLPSolver_casadi2forces)
            libc.fclose(fp)
    ...
    # convert outputs
    for out in myForcesNLPSolver_outputs:
        myForcesNLPSolver_outputs[out] = npct.as_array(getattr(outputs_py,out))

    return myForcesNLPSolver_outputs,int(exitflag),info_py

solve = myForcesNLPSolver_solve
