...
#define S_FUNCTION_NAME myForcesNLPSolver_simulinkBlock
...
static void mdlInitializeSizes(SimStruct *S)
{
	...
	/* Output Port 1 */
    ssSetOutputPortMatrixDimensions(S,  1, 153, 1);
    ssSetOutputPortDataType(S, 1, SS_DOUBLE);
    ssSetOutputPortComplexSignal(S, 1, COMPLEX_NO); /* no complex signals suppported */
	
	/* Output Port 2 */
    ssSetOutputPortMatrixDimensions(S,  2, 253, 1);
...
static void mdlOutputs(SimStruct *S, int_T tid)
{
    ...
    real_T *MVopt = (real_T*) ssGetOutputPortSignal(S,0);
	real_T *Xopt = (real_T*) ssGetOutputPortSignal(S,1);
	real_T *z = (real_T*) ssGetOutputPortSignal(S,2);
	...
	for( i=0; i<100; i++)
	{ 
		MVopt[i] = (real_T) output.MVopt[i]; 
	}

	for( i=0; i<153; i++)
	{ 
		Xopt[i] = (real_T) output.Xopt[i]; 
	}
