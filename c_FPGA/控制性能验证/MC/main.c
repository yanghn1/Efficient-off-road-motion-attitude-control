...
double dot_phi;
  ...
  double kp_num;
  double target_tmp;
  /* Initialize function 'MC' input arguments. */
  target_tmp = argInit_real_T();
  /* Call the entry-point 'MC'. */
  MC(target, x, y, theta, vx, vy,
      omega, deltaf_lastout, dist_min, &ax_next, &deltaf_next, &kp_num);
    }
 return 0;
